...
Axus は、個人のサーバーや PC で動かすファイル交換サービスである。
オープンソースで開発し、データの安全で安価な個人間共有を可能にすること、および BitTorrent の検索における単一障害点を解消することを目的とする。
Web of Trust による保護を目標に掲げる P2P ファイル共有サービスであり、daemon は 1 ノード分のネットワーク接続、ノード探索、ファイルの符号化と復号、永続状態、ローカルクライアント向け REST API を 1 プロセスにまとめる。
